...
router.post("/labels", async (req, res) => {
  ...
  if (typeof color !== "string" || !COLOR_RE.test(color)) {
    return void res.status(400).json({ error: "Color must be #RRGGBB" });
  }
  ...
  if (currentCount >= MAX_LABELS_PER_USER) {
    return void res.status(409).json({ error: `Max ${MAX_LABELS_PER_USER} labels per user` });
  }
  try {
    const [row] = await db
      .insert(labelsTable)
    ...
    if (msg.includes("labels_user_name_uidx")) {
      return void res.status(409).json({ error: "Label name already exists" });
    }
    throw err;
  }
});
...
router.post("/devices/:sessionId/chats/:waChatId/labels/:labelId", async (req, res) => {
  ...
  if (!Number.isInteger(labelId)) return void res.status(400).json({ error: "Invalid label id" });
  const [label] = await db
    .select()
    .from(labelsTable)
    .where(and(eq(labelsTable.id, labelId), eq(labelsTable.userId, ownerUserId)));
  if (!label) return void res.status(404).json({ error: "Label not found" });
  ...
  if (!chat) return void res.status(404).json({ error: "Device not found" });
  try {
