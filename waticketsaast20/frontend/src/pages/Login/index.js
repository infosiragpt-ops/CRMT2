...
const Login = () => {
	...
	const showLocalAdminLogin =
		process.env.NODE_ENV === "development" &&
		["localhost", "127.0.0.1"].includes(window.location.hostname);
	const logoWithRandom = useMemo(
		() => `${process.env.REACT_APP_BACKEND_URL}/public/logotipos/login.png?r=${Date.now()}`,
		[]
	);
	...
	const handleLocalAdminLogin = () => {
		const adminUser = { email: "admin@admin.com", password: "123456" };
		setUser(adminUser);
