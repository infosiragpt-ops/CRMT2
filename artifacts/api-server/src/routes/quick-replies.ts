...
function attachmentKind(mimetype: string): "image" | "video" | "audio" | "document" {
...
router.post(
  ...
  async (req, res) => {
    ...
    if (typeof shortcut !== "string" || !shortcut.trim()) {
      files.forEach((f) => fs.rm(f.path, () => {}));
      return void res.status(400).json({ error: "Shortcut required" });
    }
    ...
    try {
      ...
      if (msg.includes("quick_replies_user_shortcut_uidx")) {
        return void res.status(409).json({ error: "Shortcut already used" });
      }
      throw err;
    }
  },
...
router.delete("/quick-replies/:id/attachments/:attachmentId", async (req, res) => {
  ...
  const [owned] = await db
    ...
    .from(quickRepliesTable)
    .where(and(eq(quickRepliesTable.id, id), eq(quickRepliesTable.userId, ownerUserId)));
  if (!owned) return void res.status(404).json({ error: "Not found" });
  const [att] = await db
    .select()
    .from(quickReplyAttachmentsTable)
    .where(
...
router.post(
  ...
  async (req: Request, res) => {
    ...
    if (!Number.isInteger(id)) return void res.status(400).json({ error: "Invalid id" });
    const ownerUserId = await workspaceOwnerUserId(req.session.userId!);
    const device = await findDeviceBySessionForUser(req.session.userId!, sessionId);
    ...
    const attachments = await attachmentsFor(reply.id);
    await ensureChat({ deviceId: device.id, waChatId });
    ...
    try {
      ...
      if (attachments.length === 0) {
        if (reply.body.trim()) {
          results.push(
            await waManager.sendMessage(device.sessionId, waChatId, reply.body),
          );
        }
      } else {
