...
export async function ensureChat(params: {
  ...
  if (existing) {
    const nextPhoneNumber = params.phoneNumber?.replace(/\D/g, "") || "";
    const nextPhoneCode = nextPhoneNumber.length >= 6 ? nextPhoneNumber.slice(-6) : "";
    const hasVerifiedPhone = params.phoneCodeVerified === true && nextPhoneNumber.length >= 6 && nextPhoneCode.length === 6;
    if (
      (params.name && params.name !== existing.name) ||
      (params.isGroup !== undefined && params.isGroup !== existing.isGroup) ||
      (hasVerifiedPhone &&
        (existing.phoneNumber !== nextPhoneNumber ||
          existing.phoneCode !== nextPhoneCode ||
          existing.phoneCodeVerified !== true ||
          existing.phoneCodeSource !== params.phoneCodeSource))
    ) {
        ...
        .set({
          ...
          ...(hasVerifiedPhone
            ? {
                ...
                phoneCodeSource: params.phoneCodeSource ?? "contact",
              }
            : {}),
          updatedAt: new Date(),
        })
  ...
  const createdPhoneNumber = params.phoneCodeVerified === true ? params.phoneNumber?.replace(/\D/g, "") || "" : "";
  const hasCreatedVerifiedPhone = createdPhoneNumber.length >= 6;
  const [created] = await db
    ...
    .values({
      ...
      phoneNumber: hasCreatedVerifiedPhone ? createdPhoneNumber : null,
      phoneCode: hasCreatedVerifiedPhone ? createdPhoneNumber.slice(-6) : null,
      phoneCodeVerified: hasCreatedVerifiedPhone,
      phoneCodeSource: hasCreatedVerifiedPhone ? params.phoneCodeSource ?? "contact" : null,
    })
    .returning();
  return created;
}
