...
const getContactMessage = async (msg: proto.IWebMessageInfo, wbot: Session) => {
  ...
  const phoneNumber = pickPhoneNumberFromJids(
    key?.remoteJidAlt,
    key?.participantPn,
    msgAny?.senderPn,
...
const verifyContact = async (
  ...
  const contactData = {
    ...
    phoneNumber: msgContact.phoneNumber || getPhoneNumberFromJid(msgContact.id),
    lid: msgContact.lid || getLidFromJid(msgContact.id),
    profilePicUrl,
...
const wbotMessageListener = async (wbot: Session, companyId: number): Promise<void> => {
  try {
    wbot.ev.on("messages.upsert", async (messageUpsert: ImessageUpsert) => {
      ...
      logger.info(
        `messages.upsert received: ${messages.length} message(s) for company ${companyId}`
      );
