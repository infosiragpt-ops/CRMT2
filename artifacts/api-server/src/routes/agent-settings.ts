...
router.get("/agent-settings", async (req, res) => {
  ...
  return void res.json(serializeSettings(row ?? null));
});
...
router.get("/agent-settings/models", async (req, res) => {
  ...
  if (!apiKey) {
    return void res.json(modelsPayload(DEFAULT_OPENAI_MODELS, false, row?.model ?? DEFAULT_MODEL));
  }

  try {
    const models = await listOpenAiModels(apiKey);
    return void res.json(modelsPayload(models, true, row?.model ?? DEFAULT_MODEL));
  } catch (err) {
...
router.patch("/agent-settings", uploadAgentTrainingFiles.any(), async (req, res) => {
  ...
  if (rawKey) {
    ...
    if (!existingKey) {
      return void res.status(400).json({ error: "La API key guardada no se pudo leer. Pega una nueva key." });
    }
    await validateOpenAiKey(existingKey, model);
  }

  if (enabled && !encryptedKey) {
    return void res.status(400).json({ error: "Pega tu API key de OpenAI para activar el agente" });
  }
  ...
  return void res.json(serializeSettings(saved));
});
